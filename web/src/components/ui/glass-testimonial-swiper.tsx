...
export const TestimonialStack = ({ testimonials, visibleBehind = 2 }: TestimonialStackProps) => {
  ...
  return (
    <section className="testimonials-stack relative pb-10 bg-white dark:bg-black">
      {testimonials.map((testimonial, index) => {
        ...
        const tagClasses = (type: 'featured' | 'default') => type === 'featured'
          ? 'bg-white dark:bg-black text-black dark:text-white border-2 border-orange-500'
        ...
        return (
          <div
            ...
            key={testimonial.id}
            className="testimonial-card glass-effect backdrop-blur-xl bg-white/80 dark:bg-black/80 border border-black dark:border-white shadow-[0_8px_32px_rgba(0,0,0,0.1),0_0_30px_rgba(234,88,12,0.15)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.5),0_0_30px_rgba(234,88,12,0.2)] hover:shadow-[0_8px_40px_rgba(0,0,0,0.15),0_0_40px_rgba(234,88,12,0.3)]"
            ...
            <div className="p-6 md:p-8">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="flex-shrink-0 w-11 h-11 rounded-xl flex items-center justify-center bg-black dark:bg-white text-white dark:text-black font-semibold text-base shadow-lg border border-black dark:border-white">
                    {testimonial.initials}
                  </div>
                  <div>
                    <h3 className="text-black dark:text-white font-medium text-lg">{testimonial.name}</h3>
                    <p className="text-sm text-black dark:text-white mt-1">{testimonial.role}</p>
                  </div>
                </div>
              </div>

              <blockquote className="text-black dark:text-white leading-relaxed text-lg mb-6">"{testimonial.quote}"</blockquote>

              <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-t border-black dark:border-white pt-4 gap-4">
      ...
      <div className="pagination flex gap-2 justify-center absolute bottom-0 left-0 right-0">
        {testimonials.map((_, index) => (
          <button key={index} aria-label={`Go to testimonial ${index + 1}`} onClick={() => navigate(index)} className={`pagination-dot w-2 h-2 rounded-full transition-all border ${activeIndex === index ? 'active bg-orange-500 w-8 border-orange-500' : 'bg-transparent border-black dark:border-white'}`} />
        ))}
      </div>
    </section>
  );
