    ...
    <section className="testimonials-stack relative pb-10 bg-white dark:bg-black">
      {testimonials.map((testimonial, index) => {
        ...
        const tagClasses = (type: 'featured' | 'default') => type === 'featured'
          ? 'bg-white0/20 dark:bg-black/30 text-black dark:text-black border border-white0/40 dark:border-black/50'
        ...
        return (
          <div
            ...
            key={testimonial.id}
            className="testimonial-card glass-effect backdrop-blur-xl bg-white/80 dark:bg-black/80 border border-white dark:border-black shadow-xl dark:shadow-black/10"
            ...
            <div className="p-6 md:p-8">
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-4">
                  <div className="flex-shrink-0 w-11 h-11 rounded-xl flex items-center justify-center text-white font-semibold text-base shadow-lg" style={{ background: testimonial.avatarGradient }}>
                    {testimonial.initials}
                  </div>
                  <div>
                    <h3 className="text-black dark:text-white font-medium text-lg">{testimonial.name}</h3>
                    <p className="text-sm text-black dark:text-black mt-1">{testimonial.role}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-t border-white dark:border-black pt-4 gap-4">
      ...
      <div className="pagination flex gap-2 justify-center absolute bottom-0 left-0 right-0">
        {testimonials.map((_, index) => (
          <button key={index} aria-label={`Go to testimonial ${index + 1}`} onClick={() => navigate(index)} className={`pagination-dot w-2 h-2 rounded-full transition-all ${activeIndex === index ? 'active bg-black dark:bg-white0 w-8' : 'bg-white dark:bg-black'}`} />
        ))}
      </div>
    </section>
  );
